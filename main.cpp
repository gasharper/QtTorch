...
cv::Mat imgClassifier(QString modelPath, QString labPath, QString img_path, torch::Device device)
{
    ...
    std::cout<<"runtimes: "<<totaltime<<"secs"<<std::endl;
...
int detectResult(QImage& qimg)
{
    ...
    inputs.emplace_back(torch::ones({ 1, kCHANNELS, kIMAGE_SIZE, kIMAGE_SIZE }).to(device));

    // Execute the model and turn its output into a tensor.
    at::Tensor output = module.forward(std::move(inputs)).toTensor();
    std::cout << "== output slice:\n";
    std::cout << output.slice(/*dim=*/1, /*start=*/0, /*end=*/5) << '\n';

    cv::Mat result = imgClassifier("resnet50.pt", "label.txt", "shark.jpg", device);//可以自行设置测试图片和模型
